...
/**
 * Upload dist/ to:
 *   1) R2 bj-safety-bid-assets  (project bucket)
 *   2) R2 allworld-sites/bj-safety-bid/  (shared Allworld host)
...
const siteId = process.argv[2] || 'bj-safety-bid'
const projectBucket = 'bj-safety-bid-assets'
...
function put(bucket, key, file, ct) {
  ...
  execSync(
    `npx wrangler r2 object put ${bucket}/${key} --file=${JSON.stringify(file)} --content-type=${JSON.stringify(ct)}`,
